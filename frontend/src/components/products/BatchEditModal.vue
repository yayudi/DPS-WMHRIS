...
// Tabs
const activeTab = ref('export') // 'export' | 'import' | 'logs'
...
<template>
  <div v-if="isOpen"
    ...
    <div
      class="bg-[hsl(var(--color-background))] text-[hsl(var(--color-text))] rounded-xl shadow-2xl w-full max-w-4xl p-0 overflow-hidden border border-[hsl(var(--color-secondary))/0.3] flex flex-col md:flex-row h-[600px] md:h-[600px]">
      <!-- SIDEBAR -->
      <div
        class="w-full md:w-1/4 bg-[hsl(var(--color-secondary))/0.05] p-6 border-r border-[hsl(var(--color-secondary))/0.1]">
        ...
        <div class="mt-8 text-xs opacity-60 leading-relaxed">
          ...
          <div v-else-if="activeTab === 'import'">
            ...
            Upload file yang sudah diedit. Gunakan 'Test Import' untuk validasi tanpa menyimpan perubahan.
          </div>
          <div v-else>
            <p class="font-bold mb-1">Step 3:</p>
            Pantau status upload dan download laporan error jika ada data yang gagal diproses.
          </div>
        </div>
      </div>

      <!-- MAIN CONTENT -->
      <div class="w-full md:w-3/4 p-6 relative flex flex-col">
        <button @click="close"
        ...
        <div v-if="activeTab === 'export'" class="flex-1 flex flex-col">
          <h4 class="text-lg font-bold mb-1">1. Download Data Produk</h4>
        ...
        <div v-if="activeTab === 'import'" class="flex-1 flex flex-col">
          <h4 class="text-lg font-bold mb-1">2. Upload Revisi</h4>
