<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <!-- Modal Content -->
    <div class="bg-background rounded-2xl shadow-xl w-full max-w-lg overflow-hidden transform transition-all relative z-10 flex flex-col max-h-[90vh] border border-secondary/20">
      <!-- Header -->
      <div class="px-6 py-4 border-b border-secondary/20 flex justify-between items-center bg-secondary/5">
        ...
        <button @click="close" class="text-text/40 hover:text-text/80 transition-colors w-8 h-8 flex items-center justify-center rounded-full hover:bg-secondary/10">
      ...
      <div class="p-6 overflow-y-auto space-y-6">
        ...
        <div class="bg-secondary/5 border border-secondary/20 rounded-xl p-4">
          <div class="flex items-start gap-3">
             <div class="bg-secondary/20 text-text/70 w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5">
            ...
            <div class="w-full">
              ...
              >
                ...
                <div v-if="file" class="py-2">
                   <div class="w-12 h-12 bg-success/10 text-success rounded-full flex items-center justify-center mx-auto mb-2">
                  ...
                  <button @click.stop="file = null" class="text-xs text-danger hover:underline mt-2">Hapus File</button>
                </div>

                <div v-else class="py-4 space-y-2 pointer-events-none">
                  <div class="w-12 h-12 bg-secondary/10 text-text/40 group-hover:bg-primary/10 group-hover:text-primary rounded-full flex items-center justify-center mx-auto transition-colors">
      ...
      <div class="px-6 py-4 border-t border-secondary/20 bg-secondary/5 flex justify-end gap-3">
        ...
        <button
          ...
          class="px-5 py-2 text-sm font-bold text-white bg-primary hover:bg-primary/90 rounded-lg shadow-lg shadow-primary/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
