...
router.get("/", async (req, res) => {
  ...
  console.log("--- INVESTIGASI GET /products DIMULAI ---");
  ...
  try {
    ...
    console.log(
      `Parameter Parsed: location='${location}', building='${building}', floor='${floor}', minusStockOnly=${minusStockOnly}`
    );

    const allowedSortColumns = ["name", "sku", "price"];
    const safeSortBy = allowedSortColumns.includes(sortBy) ? `p.${sortBy}` : "p.name";

    // --- Pemetaan Lokasi ke Purpose ---
    let purpose = "WAREHOUSE"; // Default
    if (location === "pajangan") {
      purpose = "DISPLAY";
    } else if (location === "ltc") {
      purpose = "BRANCH";
    }
    console.log(`Hasil Pemetaan: purpose='${purpose}' (dari location='${location}')`);
    // ---------------------------------

    let baseWhereClauses = ["p.is_active = TRUE"];
    let baseParams = [];

    // --- Filter Kondisi EXISTS ---
    // Kondisi ini (EXISTS) harus selalu ada untuk memfilter produk berdasarkan 'purpose'
    // dan juga 'building'/'floor' jika dipilih.
    let existsConditions = ["l.purpose = ?"];
    let existsParams = [purpose];
    console.log("Kondisi EXISTS (existsConditions):", existsConditions);

    if (purpose === "WAREHOUSE") {
      if (building && building !== "all") {
        existsConditions.push("l.building = ?");
        existsParams.push(building);
      }
      if (floor && floor !== "all") {
        existsConditions.push("l.floor = ?");
        existsParams.push(floor);
      }
    }
    // Note: 'ltc' (BRANCH) dan 'pajangan' (DISPLAY) tidak memfilter 'building'/'floor'
    // karena mereka sudah spesifik.
    ...
    baseParams.push(...existsParams);
    // ----------------------------

    // --- Logika Search ---
    if (search) {
      baseWhereClauses.push(`${searchBy} LIKE ?`);
      baseParams.push(`%${search}%`);
    }

    // --- Logika Count (Total Produk) ---
    // Kita buat parameter terpisah untuk count agar TIDAK terpengaruh 'minusStockOnly'
    const countWhereSql = `WHERE ${baseWhereClauses.join(" AND ")}`;
    const countParams = [...baseParams];

    // --- Logika Query Produk (dengan filter minusStockOnly) ---
    let productWhereClauses = [...baseWhereClauses];
    let productParams = [...baseParams];

    if (minusStockOnly) {
      console.log("DEBUG: Filter minusStockOnly=true");
      // Kondisi untuk subquery SUM harus SAMA PERSIS dengan kondisi EXISTS
      // agar kita hanya menghitung stok dari lokasi yang relevan.
      let minusStockConditions = [...existsConditions];
      let minusStockParams = [...existsParams];

      // ✅ FIX: Tambahkan kurung ekstra di sekitar (SELECT SUM...) untuk mengatasi ER_PARSE_ERROR
      const minusStockSql = `((
            SELECT SUM(sl.quantity)
            FROM stock_locations sl
            JOIN locations l ON sl.location_id = l.id
            WHERE sl.product_id = p.id AND ${minusStockConditions.join(" AND ")}
          )) < 0`;

      productWhereClauses.push(minusStockSql);
      productParams.push(...minusStockParams);
      // 'countParams' TIDAK ditambahkan ini, sudah benar.
    }

    const productWhereSql = `WHERE ${productWhereClauses.join(" AND ")}`;
    ...
    console.log("Product SQL:", productsQuery);
    ...
    console.log(`Hasil Query: totalProduk=${totalProducts}`);

    const [products] = await db.query(productsQuery, [...productParams, limit, offset]);
    console.log(`Hasil Query: produkDitemukan=${products.length}`);
    ...
    // --- Ambil Stock Locations ---
    const productIds = products.map((p) => p.id);
    const stockLocationsQuery = `
      SELECT sl.product_id, l.code as location_code, l.building, l.floor, l.purpose, sl.quantity
      FROM stock_locations sl
      JOIN locations l ON sl.location_id = l.id
