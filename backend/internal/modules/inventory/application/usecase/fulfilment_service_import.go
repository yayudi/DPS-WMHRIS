...
func (s *fulfilmentService) ProcessSalesImport(
	...
	// For now, we will loop through parsed orders, insert basic fulfilment list data.
	...
	err = s.txManager.WithTransaction(ctx, func(ctx context.Context) error {
		...
		for _, order := range parsedOrders {
			...
			listID, err := s.fulfilmentRepo.CreateFulfilmentListTx(ctx, &domain.FulfilmentList{
			...
			for _, item := range order.Items {
				...
				err = s.fulfilmentRepo.CreateFulfilmentListItemTx(ctx, &domain.FulfilmentListItem{
					FulfilmentListID: listID,
					ProductID:     prod.ID,
