...
func NewPickingHandler(jobService system_usecase.JobService, pickingService inventory_port.PickingUseCase) *PickingHandler {
...
func (h *PickingHandler) CompleteItems(c *gin.Context) {
	req_ptr, ok := utils.BindAndValidate[inventory_dto.CompletePickingRequest](c)
	if !ok {
		return
	}
	req := *req_ptr
