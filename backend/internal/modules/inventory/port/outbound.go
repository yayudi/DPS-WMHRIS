...
type PickingRepository interface {
	...
	GetListDetails(ctx context.Context, pickingListID int) ([]inventory_dto.PickingListDetailResponse, error)

	GetHeaderByID(ctx context.Context, id int) (*domain.PickingList, error)
	GetItemsByIDs(ctx context.Context, itemIDs []int) ([]domain.PickingListItem, error)
	CountPendingItems(ctx context.Context, pickingListID int) (int, error)
	...
	ValidateItem(ctx context.Context, itemID int, locationID int) error
	ValidateHeader(ctx context.Context, listID int) error

	VoidHeader(ctx context.Context, listID int) (int64, error)
	VoidItemsByListID(ctx context.Context, listID int) error

	CreatePickingListTx(ctx context.Context, header *domain.PickingList) (int, error)
	CreatePickingListItemTx(ctx context.Context, item *domain.PickingListItem) error

	GetItemsToRestock(ctx context.Context, listID int) ([]domain.PickingListItem, error)
	GetUnfulfillableItems(ctx context.Context, listID int) ([]domain.PickingListItem, error)
	GetPendingAndBackorderItems(ctx context.Context, listIDs []int) ([]domain.PickingListItem, error)
}
...
type ReturnRepository interface {
	...
	GetPickingItemById(ctx context.Context, id int) (*domain.PickingListItem, error)
	...
	CreateSplitReturnItem(ctx context.Context, originItem *domain.PickingListItem, qtyReturn int, condition string, notes string, locationID int) (int, error)
