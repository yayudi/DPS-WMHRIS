...
type stockServiceImpl struct {
	...
	pickingRepo  inventory_repo.PickingRepository
}

func NewStockService(db *sqlx.DB, stockRepo inventory_repo.StockRepository, productRepo catalog_repo.ProductRepository, locationRepo inventory_repo.LocationRepository, userRepo iam_repo.UserRepository, pickingRepo inventory_repo.PickingRepository) StockService {
		...
		if req.ToLocationID != nil {
			...
			if err != nil {
				if errors.Is(err, sql.ErrNoRows) {
					...
					stockTo = &inventory_model.StockLocation{
...
func (s *stockServiceImpl) ProcessBatchMovements(ctx context.Context, req dto.BatchProcessRequest, userID int, userRoleID int) error {
	...
	for _, item := range resolvedItems {
		...
		case "TRANSFER", "TRANSFER_MULTI":
			...
			// Record Movement
			mov := &inventory_model.StockMovement{
				ProductID:      item.ProductID,
				Quantity:       item.Quantity,
		...
		case "ADJUSTMENT":
			...
			if stock == nil {
				stock = &inventory_model.StockLocation{
					ProductID:  item.ProductID,
					LocationID: *loc,
					Quantity:   item.Quantity,
				}
...
func (s *stockServiceImpl) ValidateReturn(ctx context.Context, req dto.ValidateReturnRequest, userID int) error {
	return database.WithTransaction(s.db, ctx, func(tx *sqlx.Tx) error {
		...
		err = s.stockRepo.RecordMovement(ctx, tx, &inventory_model.StockMovement{
			ProductID:    item.ProductID,
			Quantity:     item.Quantity,
...
func (s *stockServiceImpl) ProcessBatchOpname(ctx context.Context, tx *sqlx.Tx, movements []dto.StockOpnameItem, userID int) (int, error) {
	...
	for _, m := range movements {
		...
		history := &inventory_model.StockMovement{
			ProductID:    m.ProductID,
