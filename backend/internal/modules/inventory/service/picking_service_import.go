...
func (s *pickingService) ProcessSalesImport(
	...
	for _, order := range parsedOrders {
		...
		listID, err := s.pickingRepo.CreatePickingListTx(ctx, tx, &inventory_model.PickingList{
		...
		for _, item := range order.Items {
			...
			err = s.pickingRepo.CreatePickingListItemTx(ctx, tx, &inventory_model.PickingListItem{
