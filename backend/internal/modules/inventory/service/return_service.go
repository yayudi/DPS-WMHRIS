...
type returnServiceImpl struct {
	db           *sqlx.DB
	returnRepo   inventory_repo.ReturnRepository
	locationRepo inventory_repo.LocationRepository
	stockRepo    inventory_repo.StockRepository
}

func NewReturnService(db *sqlx.DB, returnRepo inventory_repo.ReturnRepository, locationRepo inventory_repo.LocationRepository, stockRepo inventory_repo.StockRepository) ReturnService {
...
func (s *returnServiceImpl) GetMarketplaceReturnHistory(ctx context.Context, page, limit int, search string) (utils.PaginatedResult[inventory_model.MarketplaceReturnItem], error) {
	return s.returnRepo.GetMarketplaceReturnHistory(ctx, page, limit, search)
}

func (s *returnServiceImpl) GetManualReturnHistory(ctx context.Context, page, limit int, search string) (utils.PaginatedResult[inventory_model.ManualReturnItem], error) {
...
func (s *returnServiceImpl) ApproveReturn(ctx context.Context, userID int, req dto.ApproveReturnRequest) error {
	return database.WithTransaction(s.db, ctx, func(tx *sqlx.Tx) error {
		...
		movement := &inventory_model.StockMovement{
...
func (s *returnServiceImpl) CreateManualReturn(ctx context.Context, userID int, req dto.CreateManualReturnRequest) error {
	return database.WithTransaction(s.db, ctx, func(tx *sqlx.Tx) error {
		...
		manualReturn := &inventory_model.ManualReturn{
			UserID:    userID,
