...
import (
	...
	global_service "github.com/dps-wmhris/backend/internal/service"
...
type StockRequestService interface {
	CreateStockRequest(ctx context.Context, userID int, req dto.CreateStockRequest) (*inventory_model.StockRequest, error)
	GetAllStockRequests(ctx context.Context) ([]inventory_model.StockRequest, error)
...
type stockRequestServiceImpl struct {
	db                  *sqlx.DB
	stockRequestRepo    inventory_repo.StockRequestRepository
	stockService        StockService
	notificationService global_service.NotificationService
}
...
func (s *stockRequestServiceImpl) CreateStockRequest(ctx context.Context, userID int, req dto.CreateStockRequest) (*inventory_model.StockRequest, error) {
	switch req.Type {
	case "TRANSFER":
	...
	request := &inventory_model.StockRequest{
	...
	err = database.WithTransaction(s.db, ctx, func(tx *sqlx.Tx) error {
		...
		for _, item := range req.Items {
			reqItem := &inventory_model.StockRequestItem{
...
func (s *stockRequestServiceImpl) GetAllStockRequests(ctx context.Context) ([]inventory_model.StockRequest, error) {
	...
	for i := range requests {
		...
		if err == nil {
			...
			requests[i].Items = []inventory_model.StockRequestItem{}
		}
	}
