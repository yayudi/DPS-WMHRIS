...
import (
	...
	"github.com/dps-wmhris/backend/internal/modules/iam/repository"
...
type adminUserServiceImpl struct {
	...
	systemLogRepo global_repo.SystemLogRepository
}

func NewAdminUserService(db *sqlx.DB, adminUserRepo repository.AdminUserRepository, roleRepo repository.RoleRepository, systemLogRepo global_repo.SystemLogRepository) AdminUserService {
...
func (s *adminUserServiceImpl) CreateUser(ctx context.Context, req dto.AdminCreateUserRequest, adminID int, ip, userAgent string) (*dto.AdminUserResponse, error) {
	...
	if errLog := s.systemLogRepo.Create(ctx, s.db, &global_model.SystemLog{
		UserID:     adminID,
		Action:     "CREATE",
		TargetType: "USER",
