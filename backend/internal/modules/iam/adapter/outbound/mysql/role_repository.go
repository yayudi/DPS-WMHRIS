...
func (r *roleRepositoryImpl) GetRoles(ctx context.Context) ([]iam_dto.RoleResponse, error) {
	query := "SELECT id, name, description FROM roles ORDER BY name"
	var roles []iam_dto.RoleResponse
	err := r.db.SelectContext(ctx, &roles, query)
	if err != nil {
		return nil, err
	}
	if roles == nil {
		roles = []iam_dto.RoleResponse{}
	}
	return roles, nil
}

func (r *roleRepositoryImpl) GetPermissions(ctx context.Context) ([]iam_dto.PermissionResponse, error) {
	query := "SELECT id, name, description, `group` FROM permissions ORDER BY `group`, name"
	var permissions []iam_dto.PermissionResponse
	err := r.db.SelectContext(ctx, &permissions, query)
	if err != nil {
		return nil, err
	}
	if permissions == nil {
		permissions = []iam_dto.PermissionResponse{}
	}
...
func (r *roleRepositoryImpl) DeleteRolePermissions(ctx context.Context, roleID int) error {
	query := "DELETE FROM role_permission WHERE role_id = ?"
	ext := database.GetExt(ctx, r.db)
	_, err := ext.ExecContext(ctx, query, roleID)
	return err
}

func (r *roleRepositoryImpl) InsertRolePermissions(ctx context.Context, roleID int, permissionIDs []int) error {
...
func NewRoleRepository(db *sqlx.DB) port.RoleRepository {
	base := database.NewBaseRepository[domain.Role](db, "roles")
