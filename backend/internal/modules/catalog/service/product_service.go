...
type ProductService interface {
	CreateProduct(ctx context.Context, userID int, req dto.CreateProductRequest) (*catalog_model.Product, error)
	DeleteProduct(ctx context.Context, userID int, productID int) error
	GetAllProducts(ctx context.Context) ([]catalog_model.Product, error)
...
type productServiceImpl struct {
	...
	auditRepo    catalog_repo.ProductAuditRepository
	categoryRepo catalog_repo.CategoryRepository
}

func NewProductService(db *sqlx.DB, productRepo catalog_repo.ProductRepository, auditRepo catalog_repo.ProductAuditRepository, categoryRepo catalog_repo.CategoryRepository) ProductService {
...
func (s *productServiceImpl) CreateProduct(ctx context.Context, userID int, req dto.CreateProductRequest) (*catalog_model.Product, error) {
	product := &catalog_model.Product{
		SKU:        req.SKU,
		Name:       req.Name,
	...
	err := database.WithTransaction(s.db, ctx, func(tx *sqlx.Tx) error {
		...
		log := &catalog_model.ProductAuditLog{
...
func (s *productServiceImpl) GetAllProducts(ctx context.Context) ([]catalog_model.Product, error) {
...
func (s *productServiceImpl) ProcessBatchUpdate(ctx context.Context, jobID int, filePath string, userID int, isDryRun bool) (string, error) {
	...
	productMap := make(map[string]*catalog_model.Product)
