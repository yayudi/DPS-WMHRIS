...
export class ShopeeParser {
  constructor(filePath) {
    ...
    this.source = "Shopee";
    this.mapper = Mappers["Shopee"]; // Load konfigurasi Shopee dari mappers
  ...
  // Helper: Normalisasi Header (Agresif - Copas dari TokopediaParser agar konsisten)
  _normalizeHeaderClean(val) {
    ...
    return String(val)
      ...
      .replace(/\s+/g, " ") // Ganti multiple spasi
      .trim();
  }
  ...
  async run() {
    ...
    try {
      // Shopee selalu XLSX (tapi kita handle CSV jaga-jaga)
  ...
  async _processWorkbookData(workbook) {
    ...
    // Kunci untuk deteksi header Shopee (Sesuaikan dengan importMappers.js)
    const goldenKeys = [
      "no pesanan",
      "order id",
      "nomor referensi sku",
    ...
    workbook.eachSheet((sheet) => {
      const limit = 20; // Scan 20 baris pertama
    ...
    console.log(`[ShopeeParser] 🔄 Loop Selesai. Total Rows: ${processedCount}`);
